...
def _get_neo4j_graph_data() -> dict:
    """Retourne les noeuds et arêtes pour vis.js - UNIQUEMENT les entreprises scrapées."""
    try:
        gs = GraphStore()
        nodes = []
        edges = []
        ...
        with gs.driver.session() as session:
            # Noeuds Company - UNIQUEMENT celles avec website (scrapées)
            result = session.run("""
                MATCH (c:Company)
...
def api_run(agent_name: str):
    ...
    if agent_name == "searcher":
        ...
        # Compter le nombre d'entreprises en pending
        with get_connection() as conn:
            pending_count = conn.execute(
                "SELECT COUNT(*) FROM search_results WHERE status='pending'"
            ).fetchone()[0]
        
        # Utiliser le nombre réel de pending ou la limite demandée
        limit = request.json.get("limit", pending_count) if request.json else pending_count
        logger.info(f"Scraping de {limit} entreprises (pending: {pending_count})")
